    ...
    stages {
        ...
        stage('Code Scan - Python') {
            steps{
                script {
                    sh '''
                    eval $(pylint --rcfile=pylint.cfg $(find . -type f -path '*v1/src/' -name "*.py")  --output-format=parseable -r y > pylint.log)
                    cat pylint.log
                    pylint-fail-under --fail_under 9.0 --rcfile=pylint.cfg $(find . -type f -path '*v1/src/' -name "*.py")  --output-format=parseable -r y
                    '''
                }
            }
        }
