...
def handler(event, context):
    ...
    try:
        ...
        logging.exception("DataFrameProcessingError: {}".format(e))
    ...
    try:
        ...
        logging.exception("CloseRedShiftConnection: {}".format(e))
    try:
    ...
    try:
        for key in json_obj_full:
            ...
            sns_client.publish(TopicArn='arn:aws:sns:us-east-1:332281781429:test_topic_bizcloud_sns_eventing',
                               Message=sns_message, MessageAttributes={
                    'customer_id': {
                        'DataType': 'String',
                        'StringValue': msgattr}})
    except Exception as e:
        logging.exception("FULLJsonCreateError: {}".format(e))
    try:
        for key in json_obj:
            shared_secret = get_shared_secret(json_obj[key][0]['customer_id'])
            if shared_secret is not None:
                shared_secret = bytes(shared_secret, 'utf-8')
                msgattr2 = str(json_obj[key][0]['customer_id'])
                json_body = json.dumps(json_obj[key][0])
                json_body = json_body.encode('utf-8')
                signature = hmac.new(shared_secret, json_body, hashlib.sha256).hexdigest()
                json_obj[key][0]['Signature'] = signature
                sns_message_diff = json.dumps(json_obj[key][0])
                print(sns_message_diff)
                sns_client.publish(TopicArn='arn:aws:sns:us-east-1:332281781429:test_topic_bizcloud_sns_eventing',
                                   Message=sns_message_diff,
                                   MessageAttributes={'customer_id': {'DataType': 'String', 'StringValue': msgattr2}})

    except Exception as e:
        logging.exception("DiffJSONCreateError: {}".format(e))
